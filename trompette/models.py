...
class Status(Model):
    ...
    account     = ForeignKey(Account, related_name='statuses', on_delete=CASCADE)
    boosts      = ManyToManyField(Account, related_name='boosts', through='Boost')
    created_at  = DateTimeField(auto_now_add=True)
    in_reply_to = ForeignKey('self', related_name='replies', null=True, on_delete=CASCADE)
    thread      = ForeignKey('self', related_name='+', null=True, on_delete=CASCADE)
...
class Notification(Model):
    ...
    account = ForeignKey(Account, related_name='notifications', on_delete=CASCADE)

    mention = ForeignKey(Status,  related_name='+', null=True, on_delete=CASCADE)
    # fave  = ForeignKey(Account, related_name='+')
    follow  = ForeignKey(Account, related_name='+', null=True, on_delete=CASCADE)
    at      = DateTimeField()
